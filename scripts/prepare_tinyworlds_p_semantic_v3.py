#!/usr/bin/env python3
"""Reuse pinned MiniLM evidence and construct the frozen semantic-v3 grid."""
...
def _load_evidence():
    ...
    if (
        evidence.evidence_sha256 != V3_REUSED_EVIDENCE_SHA256
        or evidence.archive_identity != CANONICAL_ARCHIVE_IDENTITY
        or evidence.encoder_identity.identity_sha256
        != ENCODER_SNAPSHOT_IDENTITY_SHA256
        or evidence.config.evidence_record() != V3_SEMANTIC_CONFIG.evidence_record()
    ):
        raise RuntimeError("cached encoder evidence does not match semantic-v3")
    return evidence
...
def _existing_catalog(evidence):
    ...
    root = SEMANTIC_DATA_ROOT / "catalog" / "v3"
    candidate_paths = (
        tuple(
            path
            ...
            == V3_SEMANTIC_CONFIG.as_record()
        )
        if root.is_dir()
        else ()
    )
    candidates = tuple(load_v3_semantic_catalog(path) for path in candidate_paths)
    matches = tuple(
        item
        for item in candidates
        if item.evidence_sha256 == evidence.evidence_sha256
        and item.encoder_identity == evidence.encoder_identity
        and item.config == V3_SEMANTIC_CONFIG
    )
    if len(matches) > 1:
        raise RuntimeError("multiple catalogs have the frozen semantic-v3 identity")
...
def _existing_failure(evidence):
    ...
    root = SEMANTIC_DATA_ROOT / "catalog" / "v3" / "failures"
    matches = []
    if root.is_dir():
        for path in sorted(root.glob("[0-9a-f]" * 64)):
            ...
            if record.get("config") != V3_SEMANTIC_CONFIG.as_record():
                continue
            artifact = load_v3_catalog_failure(path)
            if artifact.evidence_sha256 == evidence.evidence_sha256:
                matches.append(artifact)
    if len(matches) > 1:
        raise RuntimeError("multiple failures have the frozen semantic-v3 identity")
    return matches[0] if matches else None
...
def main() -> int:
    ...
    if catalog is not None:
        print(f"strict existing semantic-v3 catalog: {catalog.root}")
    ...
    working = Path(tempfile.mkdtemp(prefix="semantic-construction-v3-", dir=work_root))
    print(f"temporary artifact directory: {working}", flush=True)
    reporter = SemanticProgressReporter(
        "TinyWorlds-P semantic-v3 catalog",
        {
            "evidence-load": 20.0,
            "role-scores": 15.0,
            "calibration": 5.0,
    ...
    try:
        try:
            catalog = build_v3_catalog_from_evidence(
                evidence,
                SEMANTIC_DATA_ROOT / "catalog" / "v3",
                working / "catalog-publication",
                V3_SEMANTIC_CONFIG,
                progress=reporter,
            )
        except V3SemanticGridError:
            failure = _existing_failure(evidence)
            if failure is None:
                raise RuntimeError("semantic-v3 stopped without an authenticated audit")
            return _report_failure(failure)
    finally:
        reporter.close()
