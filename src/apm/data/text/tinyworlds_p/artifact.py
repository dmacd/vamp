...
def load_partition(path: str | Path) -> PartitionArtifact:
    ...
    if tree["format"] != "tinyworlds-p-tree" or tree["schema_version"] != 1:
    ...
    corpus_identity = _source_identity(_object(sources, "corpus"))
    metadata_identity = _source_identity(_object(sources, "metadata"))
...
def _validate_assignments(
    ...
    for line_number, record in _iter_jsonl(path):
        group_sha256 = _sha256_string(record, "normalized_sha256")
        ...
        if status == "eligible":
            ...
            if type(occurrence_ids) is not list or not occurrence_ids:
                raise PartitionArtifactError("eligible assignment lacks occurrences")
            measured_occurrences[key] += len(occurrence_ids)
            ...
            if any(record.get(field) is not None for field in ("role", "split", "world")):
...
def _validate_topology(cells: tuple[WorldCell, ...]) -> None:
    ...
    if not (
        ...
        raise PartitionArtifactError("selected cells do not form the v1 corner topology")
        ...
        canonical_record_bytes(
            {
                ...
                "sources": {
                    "corpus": corpus.as_record(),
                    "metadata": metadata.as_record(),
...
def _preset(record: dict[str, object]) -> PartitionPreset:
    ...
    return PartitionPreset(
        ...
        minimum_hash_match_coverage=_number(record, "minimum_hash_match_coverage"),
        minimum_role_coverage=_number(record, "minimum_role_coverage"),
        minimum_eligible_coverage=_number(record, "minimum_eligible_coverage"),
