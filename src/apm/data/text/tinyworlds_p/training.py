...
class StreamingTrainingConfig:
    """A validated trainer contract, fixed to v1 values in production."""
    ...
    def from_preset(
        ...
        """Expand the published v1 preset into the executable trainer contract."""
        return cls(
...
class EpochValidation:
    """Held-in and five-world validation measurements at one epoch checkpoint."""
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.world_gaps) != 5:
            raise ValueError("epoch validation requires five world gaps")
        ...
        canonical_record_bytes(
            {
                ...
                "format": "tinyworlds-p-training-resume",
...
def epoch_satisfies_gap_gates(validation: EpochValidation) -> bool:
    """Return whether one epoch can participate in final checkpoint selection."""
...
def select_best_eligible_epoch(
    ...
    """Choose lowest held-in NLL among gap-eligible epochs, breaking ties earlier."""
    eligible = tuple(
        validation
