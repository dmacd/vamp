...
def _iter_allocation_groups(
    ...
    for group in iter_archive_groups(archive_groups_path):
        ...
        canonical_occurrence = min(
            occurrences,
            key=lambda occurrence: _string(occurrence, "record_id"),
        )
        ...
        if type(raw_features) is not list or any(type(item) is not str for item in raw_features):
            raise ValueError("ingest recipe features must be a string list")
        yield AllocationGroup(
            normalized_sha256=_string(group, "normalized_story_sha256"),
    ...
    with story_spool_path.open("rb") as spool:
        for group in iter_archive_groups(ingest_path):
            if assignment is None or _string(
                ...
                raise ValueError("ingest groups and assignments are not aligned")
...
def _write_partition_metadata(
    ...
    _write_json(
        publication / "audit.json",
        {
            ...
            "archive_group_count": ingest.audit.archive_group_count,
            "archive_record_count": ingest.audit.archive_record_count,
            "duplicate_group_count": ingest.audit.duplicate_group_count,
            "exclusions": {
                "conflicting_metadata_groups": ingest.audit.conflicting_group_count,
                "empty_story_groups": ingest.audit.empty_group_count,
                "unclassifiable_metadata_groups": ingest.audit.unclassifiable_group_count,
            },
