...
def _report_text(
    ...
    return (
        "# TinyWorlds-P v1 Base Training Report\n\n"
        f"- Partition: `{artifact.partition_sha256}`\n"
        f"- Training: `{training_sha256}`\n"
        f"- Source eligible token coverage: "
        f"{_audit_eligible_coverage(artifact):.6f}\n"
...
def _audit_eligible_coverage(artifact: PartitionArtifact) -> float:
    audit = json.loads((artifact.root / "audit.json").read_bytes())
    return float(audit["source_join"]["eligible_coverage"])
...
def _write_training_tree(directory: Path, training_sha256: str) -> None:
    ...
    _write_json(
        ...
        {
            "files": files,
            "format": "tinyworlds-p-training-tree",
            "schema_version": 1,
