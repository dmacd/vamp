"""Validation-only semantic-v4 sample report publication and loading."""
...
def publish_v4_sample_report(
    artifact: V4SemanticPartitionArtifact,
    output_root: str | Path,
    temporary_directory: str | Path,
) -> V4SemanticSampleReport:
    """Publish held-in, five-world, and ten v4 control validation examples."""
    if type(artifact) is not V4SemanticPartitionArtifact:
        raise TypeError("semantic-v4 report requires its strict partition")
    ...
    content = {
        ...
        "clusters": clusters,
        "format": V4_SAMPLE_REPORT_FORMAT,
    ...
    if working.exists() and any(working.iterdir()):
        raise FileExistsError(f"semantic-v4 report work directory is not empty: {working}")
    ...
    _write_text(
        ...
        _markdown(report_sha, content, "v4"),
    )
    _write_text(
        working / "sample-report.html",
        _html(report_sha, content, "v4"),
    )
    ...
    if target.exists():
        raise FileExistsError(f"semantic-v4 sample report already exists: {target}")
    os.rename(working, target)
    _fsync_directory(partition_root)
    return load_v4_sample_report(target)


def load_v4_sample_report(path: str | Path) -> V4SemanticSampleReport:
    """Strictly authenticate a complete v4 validation-only report."""
    root = Path(path)
    if root.is_symlink() or not root.is_dir():
        raise SampleReportError("semantic-v4 report must be a regular directory")
    tree = _load_json(root / "tree.json")
    if (
        set(tree) != {"files", "format", "report_sha256", "schema_version"}
        or tree["format"] != V4_SAMPLE_REPORT_TREE_FORMAT
        or tree["schema_version"] != V4_PARTITION_SCHEMA_VERSION
    ):
        raise SampleReportError("semantic-v4 report tree changed")
    report_sha = _text(tree, "report_sha256")
    if root.name != report_sha:
        raise SampleReportError("semantic-v4 report directory identity changed")
    raw_files = tree.get("files")
    if type(raw_files) is not list or any(type(item) is not dict for item in raw_files):
        raise SampleReportError("semantic-v4 report descriptors changed")
    paths = tuple(_text(item, "name") for item in raw_files)
    if paths != ("sample-report.html", "sample-report.json", "sample-report.md"):
        raise SampleReportError("semantic-v4 report files are incomplete")
    for descriptor in raw_files:
        ...
        if (
            ...
            raise SampleReportError("semantic-v4 report file changed")
    record = _load_json(root / "sample-report.json")
    if record.get("report_sha256") != report_sha:
        raise SampleReportError("semantic-v4 report identity changed")
    content = {key: value for key, value in record.items() if key != "report_sha256"}
    if (
        record_sha256(content) != report_sha
        or record.get("format") != V4_SAMPLE_REPORT_FORMAT
        or record.get("schema_version") != V4_PARTITION_SCHEMA_VERSION
    ):
        raise SampleReportError("semantic-v4 report content identity changed")
    if record.get("sealed_test_opened") is not False:
        raise SampleReportError("semantic-v4 report opened sealed test")
    samples = record.get("samples")
    if type(samples) is not list or len(samples) != 16:
        raise SampleReportError("semantic-v4 report condition coverage changed")
    conditions = tuple(
    ...
    if conditions != expected:
        raise SampleReportError("semantic-v4 report conditions changed")
    return V4SemanticSampleReport(
        root=root.resolve(),
...
def _write_tree(root: Path, report_sha: str) -> None:
    ...
    _write_json(
        ...
        {
            "files": list(files),
            "format": V4_SAMPLE_REPORT_TREE_FORMAT,
            "report_sha256": report_sha,
            "schema_version": V4_PARTITION_SCHEMA_VERSION,
        },
    )
