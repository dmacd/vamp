...
def _plot_nll_and_gaps(reports: Path, references: dict[str, object], stages: pd.DataFrame) -> Path:
    ...
    for condition, (color, style) in NEW_STYLES.items():
        rows = stages[stages.condition == condition]
...
def _plot_resources(reports: Path, resources: pd.DataFrame) -> Path:
    ...
    for condition, rows in resources.groupby("condition", sort=False):
        ...
        color, style = NEW_STYLES[condition] if condition in NEW_STYLES else REFERENCE_STYLES[label]
    ...
    for axis, capacity in zip(axes, (1024, 4096), strict=True):
        ...
        for metric, label, style in (("interval_before", "Requested interval", "--"), ("clock_gap", "Actual interval", "-")):
            rows = histograms[(histograms.condition == condition) & (histograms.metric == metric)
                              & (histograms.kind == "historical") & (histograms.previous_quality == "all")]
            _gap_cdf(axis, rows, NEW_STYLES[condition][0], style, label)
        axis.set(xscale="log", xlabel="Scheduling ticks", ylabel="Fraction of reviews below x",
                 title=f"SRT rank 16, {capacity:,}-equivalent budget", ylim=(0, 1.02))
...
def _plot_timelines(reports: Path, timelines: pd.DataFrame) -> Path:
    figure, axes = plt.subplots(2, 1, figsize=(9.5, 8), constrained_layout=True)
    ...
    for axis, capacity in zip(axes, (1024, 4096), strict=True):
        ...
        axis.set(yticks=range(len(identities)), yticklabels=[f"Task {item.arrival_stage}: {item.image_id[:8]}" for item in identities.itertuples(index=False)],
                 ...
                 title=f"{capacity:,}-equivalent budget: SRT above / uniform below each sample row")
        axis.grid(axis="x", alpha=.2)
    return _save_figure(figure, reports / "sample_timelines.png")
...
def _sections(
    ...
    return (
        ...
        ReportSection("Full-stream comparison at both work budgets", (
        ...
        ReportSection("Carried-forward accuracy figure", (
            ...
            "true-node diagnostics for the frontier models, not deployable methods and not SRT baselines. No previous result or report was rewritten.",
        ...
        ReportSection("Probability quality and the joint-IID gap", (
            ...
            "Neither joint reference is an execution gate or a mathematical upper bound.",
        ), (figures["nll"],)),
...
def write_srt_report(run: Path) -> Path:
    ...
    for condition in NEW_STYLES:
        ...
        analyses[condition] = analyze_replay_job(run / "final" / condition, run / "final/image_index.parquet", condition)
    for capacity in (1024, 4096):
        first, second = (analyses[f"{method}_h{capacity}"] for method in ("srt", "uniform"))
        ...
        for stage in range(50):
            paired_batches = tuple(
                trace_batches(run / "final" / f"{method}_h{capacity}",
                              result["conditions"][f"{method}_h{capacity}"]["rows"][stage]["trace_chunks"])
                for method in ("srt", "uniform")
            )
    ...
    tables = {"stage_metrics": stages, "sample_replay": samples, "replay_histograms": histograms,
              ...
              "resource_metrics": resources, "task_metrics": _task_rows(result),
              "condition_names": tuple({"condition": name, "label": label} for name, label in CONDITION_LABELS.items())}
    ...
    figures = {
        ...
        "timelines": _plot_timelines(reports, pd.DataFrame(timelines)),
    }
    ...
    manifest = sealed_record({
        ...
        "reference_protocol_hash": result["protocol_hash"], "condition_names": CONDITION_LABELS,
