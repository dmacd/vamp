"""Reports for the converged full-replay Permuted-MNIST integrator ceiling."""
...
def write_phase_report(
    ...
    atomic_write(
        ...
        _html(markdown, directory, "Permuted-MNIST converged integrator ceiling").encode(
            "utf-8"
        ),
    )
...
def write_results(
    ...
    atomic_write(
        ...
        _html(markdown, run_root, "Permuted-MNIST converged integrator ceiling").encode(
            "utf-8"
        ),
    )
...
def _write_plots(
    ...
    maximum_step = max(
        (int(row["macro_step"]) for row in evaluation),
        default=0,
    )
    complete, remainder = divmod(maximum_step, config.benchmark.domain_count)
    context_steps = (config.benchmark.domain_count,) * complete + (
...
def _add_context_regions(axis: object, context_steps: tuple[int, ...]) -> None:
    ...
    for context, (start, count) in enumerate(zip(starts, context_steps, strict=True)):
        color = colors[context % len(colors)]
...
def _aggregate_markdown(
    ...
    if ceiling is None or delta_offline is None:
        return f"""# Converged full-replay integrator ceiling on Permuted-MNIST
