...
from apm.experiments.vamp_logt_router_rotated_config import RotatedTaskConfig
...
class IntegratorBenchmarkConfig:
    """Fixed Rotated-MNIST allocation and primary horizon."""
...
class VampLogTIntegratorConfig:
    """Complete direct-prediction protocol on VAMP-AF Rotated-MNIST."""
    ...
    def __post_init__(self) -> None:
        if (
            self.name != "vamp-logt-prediction-integrator-rotated-mnist"
            or self.protocol_revision != "integrated-prediction-v1"
...
def load_config(path: str | Path) -> VampLogTIntegratorConfig:
    ...
    benchmark = _mapping(
        root["benchmark"],
        "benchmark",
        {"macro_steps", "model_batch_size", "integrator_batch_size", "evaluation_batch_size"},
    )
    adapter = _mapping(
        root["adapter"],
        "adapter",
